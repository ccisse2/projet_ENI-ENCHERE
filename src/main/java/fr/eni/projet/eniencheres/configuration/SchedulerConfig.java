...
public class SchedulerConfig {
    ...
    @Autowired
    public SchedulerConfig(ArticleAVendreService articleAVendreService, EnchereService enchereService,
                           AcquisitionServiceImpl acquisitionServiceImpl) {
    ...
    @Scheduled(cron = "0 0 0 * * ?") // Vérifier toutes les minutes
    public void updateEncheresStatus() {
        ...
        for (ArticleAVendre article : articles) {
            ...
            try {
                ...
                logger.error("Erreur lors de la mise à jour de l'article {} : {}", article.getNomArticle(), e.getMessage());
            }
        }
    }
}